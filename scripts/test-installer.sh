...
if _OAV_TEST_UNAME_S=Darwin \
  ...
  fail "the Linux-only v0.1.14 release accepted a macOS target"
fi
grep -F "v0.1.14 was manually published only for Linux x86_64" \
  "${temp_root}/manual-scope.out" >/dev/null ||
  fail "the v0.1.14 manual platform scope was not explained"
