...
test("server-renders real recording controls, provider tabs, and canonical metadata", async () => {
  ...
  assert.match(html, /One dashboard for all your coding agents/);
  assert.match(html, /switch between harnesses with ease/);
  ...
  assert.match(html, /property="og:description" content="Monitor 18 coding harnesses plus Terminal, see what needs input, and jump into each native CLI from one live dashboard\."/);
  assert.match(html, /property="og:image:width" content="1200"/);
  assert.match(html, /property="og:image:height" content="630"/);
  assert.match(html, /property="og:image:alt" content="Open Agent View dashboard for 18 coding harnesses plus Terminal"/);
  ...
  assert.match(html, /data-coding-harness-count="18"/);
  assert.match(html, /aria-label="18 supported coding harnesses plus Terminal"/);
  for (const name of ["Hermes Agent", "MastraCode", "Devin"]) assert.ok(html.includes(name));
...
test("publishes a source-backed 1200 by 630 social preview", async () => {
  ...
  assert.match(source, /18 \+ TERMINAL/);
