...
| Task commands | `/help`, `enter` opens the full contextual help panel rather than a clipped footer. Verify `/harness` opens the picker; use `/harness NAME`, `/model NAME`, `/model default`, `/setup NAME`, and `/completed show|hide`, verifying the composer/header after each. `/provider` remains an alias and `/filter TEXT` remains an explicit alternative to `ctrl+f`. |
| Model picker and login | With each isolated selectable-model controller, type a task draft and press Shift+Tab. Verify catalog loading never blocks arrows/typing, the draft remains unchanged, search text stays isolated from task input, arrows/Tab wrap, Page Up/Page Down move by ten, Enter selects, and Escape preserves the prior choice/draft. Repeat through `/model`. Verify Claude aliases, cursor-paged Codex results, Pi/OpenCode `provider/model`, Cursor/Copilot/Antigravity exact account IDs, Default, empty results, catalog error, stale late result, exact typed-ID fallback only for providers that can revalidate it, and exact custom `/model NAME`. For Antigravity catalog failure, verify filter text cannot become a model and Ctrl+R retries. For a signed-out Cursor/Copilot/Antigravity fixture, both a direct task submission and Enter/`l` from the error picker must suspend OAV, show native login in a distinct setup terminal, restore OAV, automatically reload models, and preserve the draft. |
| Foreground launch | In isolated fake-account PTYs, submit every harness. Claude must return its provider-allocated short background ID and resolve the exact UUID; Codex and OpenCode must bootstrap then attach their exact managed ID; Cursor must allocate an exact chat and open interactive resume without `--print`; Pi and Copilot must pass an OAV-generated UUID, exact selected model, and initial prompt to their native UI; Antigravity must pass the exact model plus `--sandbox`; Terminal must enter the shell. The native return gesture must restore OAV and the exact new row. |
| Missing harness setup | With an isolated PATH, run `open-agent-view setup cursor` without a TTY/`--yes` and verify no downloader runs. Repeat with `--yes`; verify official URL, visible download/provider progress, private staged file execution, cleanup, and completion instructions. From the dashboard run `/setup cursor`; verify the native return gesture backgrounds a visible Terminal setup row and Enter resumes the exact login screen. |
| Manual refresh | `ctrl+l`; the dashboard reports a refresh without blocking subsequent arrow or typing input. |
| Open/return keys | Select every provider row, including managed rows with inline action capabilities, and verify both `enter` and `→` suspend the dashboard, clear the old physical screen, and open the full provider-native interface. Verify plain Left/Right move the provider cursor. At a cursor boundary, verify the first arrow shows a countdown and the same arrow returns without stopping the backend. Reopen and verify the retained screen, then exercise both Shift+Left and Shift+Right immediate return. On inline Peek, plain `←` returns to the list without starting a provider client. |
| Live indicator | Keep one Working and one Completed row visible for at least two animation intervals. The Working marker must alternate between `✳` and `·`; the Completed marker must remain `•`, navigation must stay responsive, and animation must not rebuild the cached session groups. |
...
| Composer editing | In task, filter, rename, reply, and model fields, verify Option+Backspace/Ctrl+W removes the previous Unicode-safe word and Cmd+Backspace/Ctrl+U removes to the current line start. |
| Terminal harness | Select Terminal, use a task string containing shell metacharacters as its display name, and verify they are not executed. The native return gesture backgrounds the shell, Enter/Right restores the exact screen, first Ctrl+X stops it, and second Ctrl+X deletes the completed row. |
