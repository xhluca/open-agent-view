...
| Missing harness setup | With an isolated PATH, run `open-agent-view setup cursor` without a TTY/`--yes` and verify no downloader runs. Repeat with `--yes`; verify official URL, visible download/provider progress, private staged file execution, cleanup, and completion instructions. From the dashboard run `/setup cursor`; verify Left backgrounds a visible Terminal setup row and Enter resumes the exact login screen. |
...
| Terminal harness | Select Terminal, use a task string containing shell metacharacters as its display name, and verify they are not executed. Left backgrounds the shell, Enter/Right restores the exact screen, first Ctrl+X stops it, and second Ctrl+X deletes the completed row. |
