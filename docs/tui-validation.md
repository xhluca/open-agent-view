...
| Completed visibility | Start without `--all` and verify `completed hidden`. Submit `/completed show`; the UI must stay responsive while the refreshed Completed section arrives. Submit `/completed hide`; completed rows disappear immediately and later refreshes stay active-only. Repeat with `/completed` as a toggle and with `--all` as initial shown state. |
...
| Model picker | With each isolated selectable-model controller, type a task draft and press Shift+Tab. Verify catalog loading never blocks arrows/typing, the draft remains unchanged, search text stays isolated from task input, arrows/Tab wrap, Page Up/Page Down move by ten, Enter selects, and Escape preserves the prior choice/draft. Repeat through the `/model` command. Verify Claude aliases, cursor-paged Codex results, Pi `provider/model`, OpenCode `provider/model`, Default, empty results, catalog error, stale late result, and an exact custom `/model NAME`. |
...
| Managed inline Enter | Select managed Pi/Codex/OpenCode/Cursor/Copilot rows that advertise Reply, Approve, Decline, or Respond and press `enter`; each opens Peek directly rather than handing control to a competing native client. Use an empty Peek Enter only where that provider boundary supports native open. |
...
| Local hide | Select an observe-only row from the list and from Peek, then press `ctrl+x`. Verify the confirmation says provider history/live processes are retained; cancel once and confirm once. Verify the row disappears, `sessions hidden` lists its exact ID, and `sessions unhide ID` lets provider discovery restore it. On a mixed completed group, verify only undeletable rows are offered for hiding and no provider delete is relabeled. |
