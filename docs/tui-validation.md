...
| Foreground launch | In isolated fake-account PTYs, submit Claude, Cursor, and Antigravity tasks. Claude must return its provider-allocated short background ID and resolve the exact UUID; Cursor must allocate an exact chat and open interactive resume without `--print`; Antigravity must pass the exact model plus `--sandbox`. Shift+Left must return to OAV and the exact new row must appear. |
| Missing harness setup | With an isolated PATH, run `open-agent-view setup cursor` without a TTY/`--yes` and verify no downloader runs. Repeat with `--yes`; verify official URL, visible download/provider progress, private staged file execution, cleanup, and completion instructions. From the dashboard run `/setup cursor`; verify Shift+Left backgrounds a visible Terminal setup row and Enter resumes the exact login screen. |
| Manual refresh | `ctrl+l`; the dashboard reports a refresh without blocking subsequent arrow or typing input. |
| Open/return keys | Select every provider row, including managed rows with inline action capabilities, and verify both `enter` and `→` suspend the dashboard, clear the old physical screen, and open the full provider-native interface. Verify plain Left/Right reach the provider editor, then press Shift+Left; the dashboard must return without stopping the managed backend. Reopen the same row and verify the retained screen is restored, then press Shift+Left again. On inline Peek, plain `←` returns to the list without starting a provider client. |
...
| Terminal harness | Select Terminal, use a task string containing shell metacharacters as its display name, and verify they are not executed. Shift+Left backgrounds the shell, Enter/Right restores the exact screen, first Ctrl+X stops it, and second Ctrl+X deletes the completed row. |
