...
| Open/return keys | Select every provider row, including managed rows with inline action capabilities, and verify both `enter` and `→` suspend the dashboard and open the full provider-native interface. On inline Peek, `←` returns to the list. On a Claude row, verify the footer says `ctrl+z returns`; in a safe real-provider probe, attach directly and use `ctrl+z` to return. |
