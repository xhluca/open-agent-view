...
| Open/return keys | Select rows and verify both `enter` and `→` open them. On inline Peek, `←` returns to the list. On a Claude row, verify the footer says `ctrl+z returns`; in a safe real-provider probe, attach directly and use `ctrl+z` to return. |
