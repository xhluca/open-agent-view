...
3. If the catalog says authentication is required, Enter or `l` suspends the
   ...
   `/login` exposes the same handoff explicitly. A boundary double-arrow or
   Shift+Arrow backgrounds setup as a Terminal row; it never reuses the last agent screen.
...
- Claude accepts a background launch, returns its provider-owned ID, waits for
  ...
  the native return gesture to the newly selected row.
- Antigravity begins signed out, completes its first-run handoff, reloads an
  ...
  `--prompt-interactive`, then returns with the native gesture to the exact OAV-owned cached
  conversation. The captured argv contains no dangerous permission bypass.
- A plain Terminal task opens a full-screen shell, the native return gesture backgrounds it, the
