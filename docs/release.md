...
Version 0.1.36 is the current published private preview. Hosted jobs were
unavailable, so the maintainer explicitly authorized a manual Linux x86-64
release. Its release page contains only:

```text
open-agent-view-0.1.36-x86_64-unknown-linux-gnu.tar.gz
open-agent-view-0.1.36-x86_64-unknown-linux-gnu.tar.gz.sha256
```
...
claimed for v0.1.36. Version 0.1.2 was the initial published preview. The
