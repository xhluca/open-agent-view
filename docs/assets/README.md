...
`open-agent-view.gif` and its static first frame, `open-agent-view.png`, were
...
The recorded interaction opens contextual help, filters the full queue to
Copilot, clears the filter, moves selection, and exits. Fixture mode fences all
provider I/O, so replaying the capture cannot mutate a provider session.
